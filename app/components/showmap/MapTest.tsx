...
import type {
  ...
  MapExhibition,
  ExhibitionGroup,
} from "@/lib/map/getMapData";

interface MapProps {
  searchQuery: string;
  locations: MapLocation[];
  groupedExhibitions: ExhibitionGroup[];
  embedded?: boolean;
  locale: string;
  partner: string;
}
  ...
  ({
    ...
    const getRandomLocation = (): LatLngTuple => {
      return possibleStartLocations[
        Math.floor(Math.random() * possibleStartLocations.length)
      ];
    };
    ...
    useEffect(() => {
      ...
      const newFilteredLocations: LocationWithMarker[] = locations
        .filter((location) => {
          ...
          const lon = Number(location.longitude);
          return (
            !isNaN(lat) && !isNaN(lon) && mapBounds.contains(L.latLng(lat, lon))
          );
        })
        .map((location) => ({
      ...
      setFilteredLocations((prevLocations) => {
        if (
          JSON.stringify(prevLocations) !== JSON.stringify(newFilteredLocations)
        ) {
              ...
              {filteredLocations.map((location) => {
                ...
                const locationGroups = Object.values(groupedExhibitions).filter(
                  (group) => {
                    ...
                    return (
                      group.domain === location.domain &&
                      !group.key.includes("_")
                    );
                  },
                );

                // Flatten all exhibitions from matching groups
                const exhibitionsInLocation = locationGroups.flatMap(
                ...
                return (
                  <Marker
                    ...
                    eventHandlers={{
                      ...
                      popupopen: () => {
                        if (embedded && window.umami) {
                          window.umami.track("marker_open", {
                            partner: partner || "unknown",
                            city: searchQuery || "random",
                            locale,
                      ...
                      mouseout: (e) => {
                        ...
                        setTimeout(() => {
                          ...
                          if (
                            !popup ||
                            !popup.getElement()?.matches(":hover")
                          ) {
                    ...
                    <Popup
                      ...
                      eventHandlers={{
                        ...
                        mouseout: (e) => {
                          ...
                          setTimeout(() => {
                            if (
                              !marker._popup.getElement()?.matches(":hover")
                            ) {
                              marker.closePopup();
                            }
                          }, 500); // Adjust delay as needed

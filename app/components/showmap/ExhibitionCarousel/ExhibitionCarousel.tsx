...
const ExhibitionCarousel = ({
  ...
  const trackLocationClick = (exhibition: Exhibition) => {
    if (!window.umami) return;

    window.umami.track("location_click", {
      partner: partner || "unknown",
      locale,
      location: exhibition.location || "unknown",
      title: exhibition.title || "unknown",
    });
  };
  ...
  const handleNextExhibition = () => {
    setCurrentExhibitionIndex(
      (prevIndex) => (prevIndex + 1) % exhibitionsInLocation.length,
    );
  };
  ...
  const handlePrevExhibition = () => {
    setCurrentExhibitionIndex(
      (prevIndex) =>
        (prevIndex - 1 + exhibitionsInLocation.length) %
        exhibitionsInLocation.length,
    );
  };
      ...
      {exhibitionsInLocation.map((exhibition, index) => {
        ...
        return (
          <div
            key={exhibition._id}
            style={{
              display: index === currentExhibitionIndex ? "block" : "none",
            }}
                  ...
                  {exhibition.location && exhibition.location !== "N/A" && (
                    <p className="text-sm">
                      <a
                        ...
                        onClick={() => trackLocationClick(exhibition)}
                      >
                        {exhibition.location}
                      </a>
                    </p>

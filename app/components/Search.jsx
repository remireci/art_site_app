...
const MosaicTab = dynamic(() => import("./MosaicTab"), {
  ssr: false,
  loading: () => (
    <div className="text-center py-10 text-gray-500">Loading Mosaic...</div>
  ),
});
...
const Search = ({
  ...
  useEffect(() => {
    ...
    console.log(
      "Initial data loaded via SSR. Number of loads:",
  ...
  const highlightQuery = (snippet, query) => {
    ...
    const highlightedSnippet = trimmedSnippet.replace(
      regex,
      "<strong>$&</strong>",
    ); // Wrap matched query text with <strong> tags
    ...
    <div className="main-container flex flex-wrap" id="map-container">
      ...
      <div className="w-full px-1 mb-8 mt-2 sm:w-full sm:px-1 sm:my-1 md:w-2/3 md:px-1 md:my-1 lg:px-1 lg:my-1 xl:w-2/5 ">
        <div className="flex justify-center space-x-10">
          <button
            className={`text-sm h-6 px-2 sm:mt-2  rounded ${activeTab === "list" ? "bg-slate-500 text-slate-100" : "bg-gray-200 text-gray-800 border-2 border-blue-200"} hover:bg-blue-800`}
            onClick={() => handleTabChange("list")}
          >
            {t("list")}
          </button>
          <button
            className={`text-sm h-6 px-2 sm:mt-2 rounded ${activeTab === "map" ? "bg-slate-500 text-slate-100" : "bg-gray-200 text-gray-800 border-2 border-blue-200"} hover:bg-blue-800`}
            onClick={() => handleTabChange("map")}
          >
            {t("map")}
          </button>
          <button
            className={`text-sm h-6 px-2 sm:mt-2 rounded ${activeTab === "mosaic" ? "bg-slate-500 text-slate-100" : "bg-gray-200 text-gray-800 border-2 border-blue-200"} hover:bg-blue-800`}
            onClick={() => handleTabChange("mosaic")}
          >
            {t("mosaic")}
          </button>
        </div>
        ...
        <div
          className="results-container  sm:mt-4"
        ...
        >
          {loading ? (
            ...
            <>
              ...
              {activeTab === "list" &&
                results &&
                results.filter((result) => result.source === "agenda").length >
                  0 && (
                  <ul className="w-full bg-slate-50 z-5 p-4 rounded text-xs">
                    ...
                    {results
                      .filter(
                        (result) =>
                          result.source === "agenda" &&
                          result.date_end_st !== null,
                      )
                      // .filter(result => {
                      ...
                      .map((result, index) => {
                        ...
                        if (
                          Array.isArray(result.image_reference) &&
                          result.image_reference.length > 0
                        ) {
                          ...
                          const sortedImages = result.image_reference.sort(
                            (a, b) => {
                              const getExtensionPriority = (url) => {
                                try {
                                  const ext = url
                                    ...
                                    .split(".")
                                    .pop()
                                    .toLowerCase(); // Extract clean file extension
                                  return priorityOrder.indexOf(ext) !== -1
                                    ? priorityOrder.indexOf(ext)
                                    : Infinity;
                                } catch (err) {
                                  console.error(
                                    "Failed to extract extension for:",
                                    url,
                                    err,
                                  );
                              ...
                              return (
                                getExtensionPriority(a) -
                                getExtensionPriority(b)
                              );
                            },
                          );
                        ...
                        const location = locations.find(
                          (loc) => loc.domain === result.domain,
                        );

                        const title = result.title || "";
                        const slug = location?.domain_slug;
                        const URL = slug
                          ? `${BASE_URL}/${locale}/exhibitions/locations/${slug}`
                          : result.url;
                          ...
                          >
                            {isEven ? (
                              ...
                              <>
                                <div className="flex-1">
                                  <a
                                    ...
                                    className="mt-2 block text-sm italic"
                                  >
                                    {`${result.title}`}
                                    {result.artists &&
                                      result.artists !== "N/A" && (
                                  ...
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="mt-2 text-sm block sm:inline"
                                  >
                                    {`in ${result.location}`}
                                  </a>
                                </div>
                                ...
                                >
                                  <div
                                    className="relative mt-2 sm:mt-0 sm:ml-4 flex-shrink-0 before:absolute before:top-[0px] before:left-0 before:-translate-x-52 before:w-48 before:h-[1px] before:bg-gray-400"
                              ...
                              <>
                                ...
                                >
                                  <div
                                    className="relative mt-2 sm:mt-0 sm:mr-4 flex-shrink-0 before:absolute before:top-[0px] before:right-0 before:translate-x-52 before:w-48 before:h-[1px] before:bg-gray-400"
                                    style={{ width: "160px", height: "160px" }}
                                  >
                                    <ImageDisplay
                                      imagePath={imagePath}

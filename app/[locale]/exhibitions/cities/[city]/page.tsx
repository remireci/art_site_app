...
export async function generateMetadata({
  ...
  try {
    messages = (
      await import(`../../../../../locales/${locale}/exhibitions.json`)
    ).default;
  } catch (error) {
    // Fallback to English if locale messages are not found
    messages = (await import(`../../../../../locales/en/exhibitions.json`))
      .default;
  }
...
export default async function CityPage({
  ...
  const validCity = exhibitions.find(
        ...
        {exhibitions.length > 0 ? (
          ...
          <div>
            <h1 className="text-2xl text-gray-700 font-semibold mb-4">
              {`${messages.cities.noExhibitions}`}{" "}
              {city || slug.slice(0, 1).toLocaleUpperCase() + slug.slice(1)}
            </h1>
            <p className="max-w-2xl text-gray-600">
              While we couldn&apos;t find any exhibitions happening in{" "}
              {slug.slice(0, 1).toUpperCase() +
                slug.slice(1).toLocaleLowerCase()}{" "}
              right now, the art scene is always evolving. Check back soon or
              explore exhibitions in nearby cities. You can also use our
              interactive map to discover art events across Europe.
            </p>
          </div>
        )}
